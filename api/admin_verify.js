export default async function handler(req, res) {
  ...
  const { action, password, payment_id } = req.body;
  ...
  if(action === 'login'){
    return res.status(200).json({ ok: password === ADMIN_PASS });
  }
  ...
  if(action === 'verify'){
    await fetch(SUPABASE_URL + '/rest/v1/payments?payment_id=eq.' + payment_id, {
      method: 'PATCH',
      headers,
